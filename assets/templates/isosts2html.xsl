...
<!-- ============================================================= -->  
  ...
  <!-- Match and copy the mml:math element and its contents -->
  <xsl:template match="mml:math">
    <math style="font-size: 150%;" xmlns="http://www.w3.org/1998/Math/MathML" id="{@id}">
      <xsl:apply-templates/>
    </math>
  </xsl:template>

  <!-- Copy all child elements of mml:math (like mml:mi, mml:mo, mml:mfrac, etc.) -->
  <xsl:template match="mml:*">
    <xsl:element name="{local-name()}">
      <xsl:copy-of select="@*"/> <!-- Copy attributes like id, mathvariant, etc. -->
      <xsl:apply-templates/>
    </xsl:element>
  </xsl:template>
  <!-- ============================================================= -->
...
<xsl:template match="committee">
  <xsl:variable name="committeePosition" select="position()" />
  <div id="committee-{position()}" style="margin-top: 2px; font-size: 10px; background-color: #f0f0f0;">
    <h3 id="committee-title-{position()}" style="text-align: center;">
      <xsl:value-of select="title" />
    </h3>
    <table id="committee-table-{position()}" cellpadding="2" cellspacing="0">
      <thead>
        <tr>
          <th></th>
          <th style="text-align: left;">성명</th>
          <th style="text-align: left;">근무처</th>
          <th style="text-align: left;">직위</th>
        </tr>
      </thead>
      <tbody style="font-size: 10px;">
        <xsl:for-each select="member">
          <xsl:if test="normalize-space(name) != '' and name != '&#160;'">
            <tr id="committee-{$committeePosition}-member-{position()}">
              <td style="font-size: 10px;" id="committee-{$committeePosition}-member-role-{position()}">
                <xsl:choose>
                  <xsl:when test="role">
              ...
              <td style="font-size: 10px;" id="committee-{$committeePosition}-member-name-{position()}"><xsl:value-of select="name" /></td>
              <td style="font-size: 10px;" id="committee-{$committeePosition}-member-affiliation-{position()}"><xsl:value-of select="affiliation" /></td>
              <td style="font-size: 10px;" id="committee-{$committeePosition}-member-position-{position()}"><xsl:value-of select="position" /></td>
            </tr>
          </xsl:if>
        </xsl:for-each>
      </tbody>
...
<xsl:template match="ksStandardAccess">
  <div id="ksStandardAccess" style="margin-top: 2px; font-size: 10px; background-color: #f0f0f0;">
    <h3 style="text-align: center;">
      <span id="access-title"><xsl:value-of select="title"/></span>
      <span id="access-url">(<a href="{url}" target="_blank"><xsl:value-of select="url"/></a>)</span>
    </h3>
    <table id="access-info" class="info-table">
      <tr id="issuer">
        <td>제 정 자:</td>
        <td><xsl:value-of select="details/issuer"/></td>
      </tr>
      <tr id="department">
        <td>담당부처:</td>
        <td><xsl:value-of select="details/department"/></td>
      </tr>
      <tr id="issuedDate">
        <td>제 정:</td>
        <td><xsl:value-of select="details/issuedDate"/></td>
      </tr>
      <tr id="confirmationDate">
        <td>확 인:</td>
        <td><xsl:value-of select="details/confirmationDate"/></td>
      </tr>
      <tr id="committee">
        <td>심 의:</td>
        <td><xsl:value-of select="details/committee"/></td>
      </tr>
      <tr id="cooperation">
        <td>원안작성협력:</td>
        <td><xsl:value-of select="details/cooperation"/></td>
      </tr>
    </table>
    <div id="footer" class="footer">
      <xsl:value-of select="notes/comment"/><br/><br/>
      <xsl:value-of select="notes/updateRule"/>
    </div>
  </div>
</xsl:template>

<xsl:template match="ksStandard/cover">
  <div id="ks-cover" style="margin-top: 2px; font-size: 10px; background-color: #f0f0f0;">
    <p id="ks-standard-id"><xsl:value-of select="standardId"/></p>
    <h1 id="ks-title"><xsl:value-of select="title"/></h1>
    <h1 id="ks-version"><xsl:value-of select="standardId"/>:<xsl:value-of select="version"/></h1>
    <h1 id="ks-publisher"><xsl:value-of select="publisher"/></h1>
    <p id="ks-date"><xsl:value-of select="date"/></p>
  </div>
</xsl:template>


  <xsl:template match="ksStandardTextInfo"/>
